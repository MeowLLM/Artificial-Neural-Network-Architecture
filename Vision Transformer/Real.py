...
# --- NEW: Multi-Head Latent Attention Block ---
class LatentAttentionBlock(nn.Module):
    """
    Implements a block that uses Multi-Head Cross-Attention to project input
    features into a smaller set of learned latent queries, followed by an MLP.
    This effectively compresses the information from a longer input sequence
    into a shorter latent sequence.
    """
    def __init__(self, input_embed_dim: int, num_latents: int, num_heads: int, mlp_dim: int, dropout_rate: float = 0.1):
        """
        Initializes the LatentAttentionBlock.

        Args:
            input_embed_dim (int): The dimension of the input sequence embeddings.
            num_latents (int): The number of learnable latent queries.
            num_heads (int): Number of attention heads for cross-attention.
            mlp_dim (int): Hidden dimension for the MLP block applied to latents.
            dropout_rate (float): Dropout rate.
        """
        super().__init__()
        ...
        # Learnable latent queries. These act as the 'queries' in cross-attention,
        # aggregating information from the input sequence.
        self.latent_queries = nn.Parameter(torch.randn(1, num_latents, input_embed_dim))

        # Layer Normalization for the input sequence (keys/values)
        self.norm_input = nn.LayerNorm(input_embed_dim)
        # Layer Normalization for the latent queries (queries)
        self.norm_latent = nn.LayerNorm(input_embed_dim)

        # Multi-Head Cross-Attention: queries come from latents, keys/values from input
        self.cross_attn = nn.MultiheadAttention(input_embed_dim, num_heads, dropout=dropout_rate, batch_first=True)
        self.dropout_attn = nn.Dropout(dropout_rate)

        # MLP block applied to the latent representations after cross-attention
        self.mlp_block = MLPBlock(input_embed_dim, mlp_dim, dropout_rate)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Forward pass of the LatentAttentionBlock.

        Args:
            x (torch.Tensor): Input tensor (e.g., patch embeddings) of shape (B, S_input, E).

        Returns:
            torch.Tensor: Output latent representations of shape (B, num_latents, E).
        """
        B = x.shape[0]

        # Expand latent queries to match the batch size
        latents = self.latent_queries.expand(B, -1, -1)

        # Apply Layer Normalization to input and latents before cross-attention
        norm_x = self.norm_input(x)
        norm_latents = self.norm_latent(latents)

        # Perform Multi-Head Cross-Attention
        # Query: normalized latents, Key/Value: normalized input (patches + CLS)
        attn_output, _ = self.cross_attn(query=norm_latents, key=norm_x, value=norm_x)
        attn_output = self.dropout_attn(attn_output)

        # Add residual connection for latents
        latents = latents + attn_output

        # Pass the updated latent representations through an MLP block
        # The MLPBlock includes its own LayerNorm and residual connection
        latents = self.mlp_block(latents)

        return latents

# --- 6. EfficientVisionTransformer Model (Modified for Latent Attention) ---
class EfficientVisionTransformer(nn.Module):
    """
    Implements the Efficient Vision Transformer model with Multi-Head Latent Attention.
    It processes images through patch embedding, projects them into a smaller
    set of latent tokens via cross-attention, processes these latents with
    standard transformer blocks, and then uses a DeepGatedMLP for final classification.
    """
    def __init__(self, img_size: int = 224, patch_size: int = 16, in_channels: int = 3,
                 embed_dim: int = 768, num_latents: int = 64, num_latent_transformer_blocks: int = 8,
                 num_heads: int = 12, mlp_dim_ratio: int = 4, num_classes: int = 100,
                 dropout_rate: float = 0.1, gated_mlp_num_layers: int = 2):
        """
        Initializes the EfficientVisionTransformer with Latent Attention.

        Args:
            img_size (int): Size of the input image (e.g., 224 for 224x224).
            patch_size (int): Size of image patches (e.g., 16 for 16x16 patches).
            in_channels (int): Number of input channels (e.g., 3 for RGB).
            embed_dim (int): Dimension of patch embeddings and latent transformer hidden states.
            num_latents (int): Number of learned latent tokens to project the input into.
            num_latent_transformer_blocks (int): Number of Transformer Encoder Blocks
                                                 applied to the latent sequence.
            num_heads (int): Number of attention heads in MultiHeadSelfAttention and Cross-Attention.
        ...
        # 2. Initial Latent Attention Block (Cross-Attention from latents to patches)
        # This block takes the patch embeddings and converts them into 'num_latents'
        # compressed latent representations.
        self.latent_projection_block = LatentAttentionBlock(
        ...
        # 3. Transformer Blocks operating on the latent sequence (Self-Attention among latents)
        # These blocks further process the compressed latent representations.
        self.latent_transformer_blocks = nn.ModuleList([
            TransformerBlock(
                ...
                mlp_dim=self.mlp_dim,
                dropout_rate=dropout_rate
            ) for _ in range(num_latent_transformer_blocks)
        ])

        # 4. Final Layer Normalization before classification head
        self.norm_final = nn.LayerNorm(embed_dim)

        # 5. Classification Head: DeepGatedMLP and a standard MLP in parallel
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Forward pass of the EfficientVisionTransformer with Latent Attention.
        ...
        # 2. Project into latent space using Latent Attention Block
        # This performs cross-attention to get 'num_latents' tokens.
        # Output shape: (B, num_latents, embed_dim)
        latent_tokens = self.latent_projection_block(x)

        # 3. Pass the latent tokens through subsequent Transformer Blocks (self-attention)
        for block in self.latent_transformer_blocks:
            latent_tokens = block(latent_tokens)

        # 4. Extract the first latent token for classification
        # We assume the first latent token acts as the aggregated representation
        # similar to a CLS token in standard ViTs.
        cls_latent_output = latent_tokens[:, 0]

        # 5. Apply final Layer Normalization
        cls_latent_output = self.norm_final(cls_latent_output)

        # 6. Classification with DeepGatedMLP head
        logits, _ = self.gated_mlp_head(cls_latent_output)

        # You could uncomment and use the standard MLP head or combine them if needed:
        # logits_gated, _ = self.gated_mlp_head(cls_latent_output)
        # logits_standard = self.standard_mlp_head(cls_latent_output)
